...
export async function migrate(modules) {
  try {
    ...
    /**
     * Safety: DB connection/migrations should never prevent the HTTP server from starting.
     *
     * What was wrong:
     * - A transient DB outage or misconfiguration caused a hard `process.exit(0)` during boot.
     *
     * Why this fix is safe:
     * - We keep all migration logic intact and still log the error.
     * - We only prevent the *crash*, allowing the app to start so operators can fix DB
     *   without the process repeatedly dying.
     */
    error(e);
  }
}
